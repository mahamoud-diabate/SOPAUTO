...
if getattr(sys, 'frozen', False):
    ...
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # up from db/
...
# Connexion persistante — évite d'ouvrir/fermer le fichier à chaque requête (×27 plus rapide)
_conn_persistante = None
...
def get_connection() -> sqlite3.Connection:
    """Retourne la connexion persistante (créée au 1er appel, ×27 plus rapide)."""
    global _conn_persistante
    if _conn_persistante is None:
...
def close_connection() -> None:
    """Ferme la connexion persistante — appelé à la fermeture de l'application."""
    global _conn_persistante
    if _conn_persistante:
        _conn_persistante.close()
        _conn_persistante = None
