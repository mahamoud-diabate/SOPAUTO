...
def enregistrer_prix(conn, produit_id: int, type_prix: str, ancien: float,
    ...
    if abs(_num(ancien) - _num(nouveau)) < 0.01:
        return
    conn.execute(
        """INSERT INTO prix_historique
           ...
           VALUES (?,?,?,?,?,?,?,?,?)""",
        (produit_id, type_prix, _num(ancien), _num(nouveau), origine, tiers,
...
def calculer_cump(stock_actuel: int, cump_actuel: float,
    ...
    cump_actuel = _num(cump_actuel)
    prix_entree = _num(prix_entree)
    if qte_entree <= 0:
        return cump_actuel
...
def _maj_cump(conn, produit_id: int, qte_entree: int, prix_entree: float,
    ...
    if not prod:
        return 0.0
    ancien = _num(prod["cump"]) or _num(prod["prix_achat"])
    ...
    conn.execute("UPDATE produits SET cump=?, prix_achat=?, date_dernier_achat=? WHERE id=?",
                 (round(nouveau, 2), _num(prix_entree), _maintenant(), produit_id))
        ...
        with conn:
            ...
            conn.execute(
                """INSERT INTO mouvements_stock
                   ...
                   VALUES (?,'entree',?,?,?,?,?,?,?,?,?,?)""",
                (produit_id, quantite, _num(prix_unitaire), reference_doc, notes,
...
def verifier_plafond_credit(client_id: int | None, montant: float) -> tuple[bool, str]:
    ...
    plafond = _num(row["plafond_credit"]) or _num(params.get("credit_plafond_defaut", 0))
    if plafond <= 0:
    ...
    if encours + _num(montant) > plafond:
        return False, (f"Plafond dépassé pour « {row['nom']} » : encours {encours:,.0f} "
                       f"+ {_num(montant):,.0f} > plafond {plafond:,.0f}")
...
def creer_commande(fournisseur_id: int, items: list, depot_id: int | None = None,
    ...
    for pid, designation, qte, pu in items:
        ...
        lignes.append((pid, (designation or "").strip(), qte, _num(pu)))
        ...
        with conn:
            cur = conn.execute(
                """INSERT INTO commandes (fournisseur_id, depot_id, statut, sous_total,
                   ...
                   VALUES (?,?,'brouillon',?,?,?,?,?,?,?,?)""",
                (fournisseur_id, depot_id, sous_total, remise, _num(frais), total,
            ...
            conn.execute(
                """UPDATE inventaire_lignes
                   SET stock_compte=?, ecart=?, valeur_ecart=?, motif=?, notes=?,
                       date_comptage=?
                   WHERE inventaire_id=? AND produit_id=?""",
                (stock_compte, ecart, ecart * _num(ligne["cump_unitaire"]), motif, notes,
...
def cloturer_inventaire(inventaire_id: int, appliquer: bool = True) -> tuple[bool, str]:
    ...
    try:
        with conn:
            ...
            for ligne in lignes:
                nb_ecarts += 1
                valeur_ecart += _num(ligne["valeur_ecart"])
                if not appliquer:
                    continue
    ...
    try:
        with conn:
            ...
            for item in items:
                ...
                lignes.append((pid, qte, _num(pu), 1 if remis else 0, etat))
    ...
    for r in rows:
        ...
        if couverture <= horizon_jours or (stock <= (r["stock_mini"] or 0) and vitesse > 0):
            resultat.append({
                ...
                "valeur_commande": round(besoin * _num(r["cump"]), 2),
...
def calculer_classes_abc(fenetre_jours: int = 180) -> tuple[bool, str]:
    ...
    try:
        ...
        total = sum(_num(r["ca"]) for r in rows)
        if total <= 0:
...
def kpi_v3() -> dict:
    ...
    def q(sql, params=()):
        ...
        return _num(row[0]) if row and row[0] is not None else 0.0

    params_app = get_parametres()
    seuil = int(_num(params_app.get("alerte_creance_jours", 15)) or 15)
